    ...
    cy.contains('.ant-card-head-title', 'Editing Task')
      ...
      .within(() => {
        ...
        cy.get('input').eq(1)
          ...
          // กดที่ตัว selector (ไม่ใช่ .ant-select ตรงๆ)
    cy.get('.ant-select-selector').click();
  });

// รอให้ dropdown โผล่ใน body แล้วเลือก “Completed”
cy.get('body')
  .find('.ant-select-dropdown')
  .should('be.visible')
  .contains('.ant-select-item-option-content', 'Completed')
  .click();

// สุดท้ายกลับไปกด Save ใน Card เดิม
cy.contains('.ant-card-head-title', 'Editing Task')
  .closest('.ant-card')
  .within(() => {
