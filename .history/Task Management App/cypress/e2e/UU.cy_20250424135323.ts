it('Creates a task, clicks Edit, and updates it in the "Editing Task" section', () => {
  ...
  cy.contains('Editing Task')
    ...
    .within(() => {
      cy.get('input[placeholder="Title"]').clear().type('Pondza');
      cy.get('input[placeholder="Description"]').clear().type('This task was modified via test.');
  ...
  cy.contains('This task was modified via test.').should('exist');
