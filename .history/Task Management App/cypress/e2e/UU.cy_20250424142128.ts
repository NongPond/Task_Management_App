it('Creates a task, clicks Edit, and updates it in the "Editing Task" section', () => {
  ...
  cy.contains('Editing Task')
    .parent() // Use parent() or closest() to make sure we're within the correct section
    .within(() => {
      ...
      cy.get('.ant-select').click(); // Click to open the status dropdown
      cy.get('.ant-select-item-option-content').contains(updatedStatus).click(); // Select status

      cy.contains('Save').click(); // Save the changes
    });

  // Step 8: Wait for the update task request to finish
  cy.wait('@updateTask');

  // Step 9: Verify the updated task appears in the task list
  cy.contains(updatedTitle).should('exist');
  cy.contains(updatedDescription).should('exist');
  cy.contains(updatedStatus).should('exist'); // Verify the updated status is displayed
});
