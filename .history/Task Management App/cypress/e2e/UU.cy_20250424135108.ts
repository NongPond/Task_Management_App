it('Creates a task, clicks Edit, and updates it in the "Editing Task" section', () => {
  ...
  cy.wait('@createTask');
    ...
    .first()
    .click();
  // Focus on Editing Task section
  cy.contains('Editing Task')
    .parent() // หรือ .closest('div') ถ้าจำเป็น
    .within(() => {
      cy.get('input[placeholder="Title"]').clear().type('✅ Edited Task');
      cy.get('input[placeholder="Description"]').clear().type('Updated description of the task.');
  ...
  cy.contains('Updated description of the task.').should('exist');
