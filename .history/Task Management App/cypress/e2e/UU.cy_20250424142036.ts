it('Creates a task, clicks Edit, and updates it in the "Editing Task" section', () => {
  ...
  const updatedStatus = 'Completed'; // เปลี่ยนสถานะเป็น Completed
  ...
  cy.contains('Editing Task')
    ...
    .within(() => {
      // Wait for and update Title, Description, and Status fields
      ...
      cy.get('.ant-select-item-option-content').contains(updatedStatus).click(); // Select Completed status
  ...
  cy.contains(updatedStatus).should('exist'); // Verify that the updated status is displayed
});
