it('Creates a task and edits it', () => {
  cy.visit('/');
  ...
  cy.get('input').first().clear().type(updatedTitle); // Update the title
  cy.contains('Save').click(); // Save the changes
  cy.wait('@updateTask');

  // Step 4: Verify the updated task
  cy.contains(updatedTitle).should('exist'); // Ensure the updated task title is displayed
});
