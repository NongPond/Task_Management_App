it('Creates a task, clicks Edit, and updates it in the "Editing Task" section', () => {
  ...
  cy.get('input[placeholder="Title"]').clear().type(updatedTitle); // Clear and enter the updated title
  cy.get('input[placeholder="Description"]').clear().type(updatedDescription); // Clear and enter the updated description
