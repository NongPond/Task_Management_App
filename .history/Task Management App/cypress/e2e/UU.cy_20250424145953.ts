it('สร้าง task แล้ว edit เพื่อเปลี่ยนข้อความใน Editing Task ได้สำเร็จ', () => {
  ...
  const taskTitle = 'Task เดิม'; // ข้อความเดิม
  const taskDescription = 'Description เดิม'; // ข้อความเดิม
  ...
  const updatedTitle = 'Task ที่แก้ไขแล้ว'; // ข้อความใหม่ที่จะเปลี่ยน
  const updatedDescription = 'Description ที่เปลี่ยนแล้ว'; // ข้อความใหม่ที่จะเปลี่ยน
  const updatedStatus = 'Completed';

  // Step 5: Ensure the "Editing Task" section is visible
  cy.contains('Editing Task').should('be.visible');

  // Step 6: Edit the Title in the "Editing Task" section (ห้ามไปใส่ข้อความใหม่ที่ช่องเพิ่ม task)
  cy.get('input[placeholder="Title"]')
    .should('be.visible')
    .clear()  // ลบข้อความเดิมทั้งหมดในช่อง Title
    .type(updatedTitle)  // พิมพ์ข้อความใหม่ที่ต้องการ
    .should('have.value', updatedTitle);  // ตรวจสอบว่า Title ถูกอัปเดตเป็นข้อความใหม่

  // Step 7: Edit the Description
  cy.get('input[placeholder="Description"]')
    .should('be.visible')
    .clear()  // ลบข้อความเดิมทั้งหมดในช่อง Description
    .type(updatedDescription)  // พิมพ์ข้อความใหม่ที่ต้องการ
    .should('have.value', updatedDescription);  // ตรวจสอบว่า Description ถูกอัปเดตเป็นข้อความใหม่
