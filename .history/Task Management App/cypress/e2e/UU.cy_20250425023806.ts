    ...
    cy.contains('.ant-card-head-title', 'Editing Task')
      ...
      .within(() => {
        ...
        cy.get('input').eq(1)
         ...
         // 3. เปิด Select ด้วย selector และรอให้ dropdown พร้อม
    cy.get('.ant-select-selector').click({ force: true });

    // รอให้ dropdown แสดง (ลองเช็คความพร้อมของ .ant-select-dropdown)
    cy.get('.ant-select-dropdown')
      .should('be.visible');  // ให้มั่นใจว่า dropdown ปรากฏแล้ว

    // 4. พิมพ์ 'Completed' ลงในช่อง search ของ select แล้วกด Enter
    cy.get('.ant-select-selection-search-input')
      .type('Completed{enter}', { force: true });
        ...
        cy.contains('.ant-card', 'Completed').should('exist');
});
