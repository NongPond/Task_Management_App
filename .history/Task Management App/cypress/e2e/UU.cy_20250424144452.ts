...
cy.contains('Editing Task')
  .parent() // หรือ .parentsUntil('.some-known-container')
  .within(() => {
    cy.get('input[placeholder="Title"]')
      .click()
      .type('{selectall}{backspace}')
      .type('Task ที่แก้ไขแล้ว')
      .should('have.value', 'Task ที่แก้ไขแล้ว');

    cy.get('input[placeholder="Description"]')
      .click()
      .type('{selectall}{backspace}')
      .type('Description ที่เปลี่ยนแล้ว')
      .should('have.value', 'Description ที่เปลี่ยนแล้ว');
