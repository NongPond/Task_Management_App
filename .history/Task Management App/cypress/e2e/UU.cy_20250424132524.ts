it('Creates a task, clicks Edit, and updates it in the "Editing Task" section', () => {
  ...
  // Step 4: Click Edit on the created task
  cy.contains(taskTitle).parent().contains('Edit').click(); // Target edit button of correct task

  // Step 5: Check initial values in Editing Task form
  cy.get('input[placeholder="Title"]').should('have.value', taskTitle);
  cy.get('input[placeholder="Description"]').should('have.value', taskDescription);
