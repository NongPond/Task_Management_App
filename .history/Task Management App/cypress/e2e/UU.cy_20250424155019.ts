it('สร้าง task แล้ว edit เพื่อเปลี่ยนข้อความใน Editing Task ได้สำเร็จ', () => {
  ...
  // แก้ไขข้อความของ Task
  const newTaskTitle = 'Task ใหม่';
  const newTaskDescription = 'Description ใหม่';
  ...
  // ตรวจสอบว่า Task ถูกแก้ไขเรียบร้อย
  cy.contains(newTaskTitle).should('exist');
  cy.contains(newTaskDescription).should('exist');
});
