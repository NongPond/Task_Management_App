...
describe("Register component", () => {
  ...
  it("shows default error message when no message from backend", async () => {
    ...
    await waitFor(() => {
      // ใช้ Non-null Assertion (!) เพื่อให้ TypeScript รู้ว่า element ไม่เป็น null
      expect(screen.getByText((content, element) => 
        content.includes("กรุณากรอกอีเมล") && element!.tagName.toLowerCase() === 'div'
      )).toBeInTheDocument();
  
      expect(screen.getByText((content, element) => 
        content.includes("กรุณากรอกรหัสผ่าน") && element!.tagName.toLowerCase() === 'div'
      )).toBeInTheDocument();
    });
  });  
