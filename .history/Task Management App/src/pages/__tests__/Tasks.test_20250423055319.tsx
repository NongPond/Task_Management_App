...
describe('Tasks Component', () => {
  ...
  // Persist to localStorage after load
  ...
  // Updates localStorage on create
  it('updates localStorage when a new task is created', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /create/i }));
    expect(await screen.findByText(/new task/i)).toBeInTheDocument();
    const storedArr = JSON.parse(localStorage.getItem(`tasks-${email}`) || '[]');
    expect(storedArr).toHaveLength(2);
    expect(storedArr[1]).toMatchObject({ title: 'New Task', description: 'New Desc', status: 'pending' });
  });
