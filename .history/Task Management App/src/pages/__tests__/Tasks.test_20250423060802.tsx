...
describe('Tasks Component', () => {
  ...
  it('falls back to localStorage data when API fetch fails', async () => {
    ...
    const saved = [{ _id: 'x', title: 'Saved Task', description: 'Saved', status: 'pending' }];
    localStorage.setItem(`tasks-${email}`, JSON.stringify(saved));
    ...
    // Should still render saved tasks and show error alert
  ...
  it('creates a task', async () => {
    renderTasksComponent();
    fireEvent.change(await screen.findByPlaceholderText(/title/i), {
  ...
  it('prevents creating task with empty fields', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent(
      /both title and description are required/i
    );
  });
  ...
  it('updates localStorage when a task is updated', async () => {
    ...
    await waitFor(() => {
      const stored = JSON.parse(
        localStorage.getItem(`tasks-${email}`) || '[]'
      );
  ...
  it('updates task status to "completed"', async () => {
    ...
    expect(
      await screen.findByText((c) => c.toLowerCase().includes('completed'))
    ).toBeInTheDocument();
  });
