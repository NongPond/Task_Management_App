...
describe('Tasks Component', () => {
  ...
  it('shows message when no tasks found', async () => {
    server.use(
      rest.get(`http://localhost:5000/api/tasks/${email}`, (_, res, ctx) => res(ctx.json([])))
    );
    render(
      <MemoryRouter>
        <Tasks />
  ...
  it('shows error message on fetch failure', async () => {
    server.use(
      rest.get(`http://localhost:5000/api/tasks/${email}`, (_, res, ctx) => res(ctx.status(500)))
    );
    render(
      <MemoryRouter>
        <Tasks />
  ...
  it('renders multiple tasks when API returns many', async () => {
    ...
    server.use(
      rest.get(`http://localhost:5000/api/tasks/${email}`, (_, res, ctx) => res(ctx.json(twoTasks)))
    );
    render(
      <MemoryRouter>
        <Tasks />
  ...
  it('handles delete failure if task not found', async () => {
    server.use(
      rest.delete('http://localhost:5000/api/tasks/:id', (_, res, ctx) => res(ctx.status(404)))
    );
    render(<MemoryRouter><Tasks /></MemoryRouter>);
    fireEvent.click(await screen.findByRole('button', { name: /delete/i }));
    const alert = await screen.findByRole('alert');
  ...
  it('updates task status to "in-progress"', async () => {
    ...
    const status = await screen.findByText((c) => /in[-\s]?progress/i.test(c));
  ...
  it('updates task status to "completed"', async () => {
    ...
    const status = await screen.findByText((c) => c.toLowerCase().includes('completed'));
    expect(status).toBeInTheDocument();
  });
  ...
  it('shows error when create task fails', async () => {
    server.use(
      rest.post('http://localhost:5000/api/tasks', (_, res, ctx) => res(ctx.status(500)))
    );
    render(<MemoryRouter><Tasks /></MemoryRouter>);
  ...
  it('persists tasks to localStorage after initial load', async () => {
    render(<MemoryRouter><Tasks /></MemoryRouter>);
    expect(await screen.findByText('Test Task')).toBeInTheDocument();
    const stored = localStorage.getItem(`tasks-${email}`);
    expect(stored).toEqual(JSON.stringify(mockTasks));
  });

  it('updates localStorage when a new task is created', async () => {
    render(<MemoryRouter><Tasks /></MemoryRouter>);
    fireEvent.change(await screen.findByPlaceholderText(/title/i), { target: { value: 'New Task' } });
    fireEvent.change(screen.getByPlaceholderText(/description/i), { target: { value: 'New Desc' } });
    fireEvent.click(screen.getByRole('button', { name: /create/i }));
