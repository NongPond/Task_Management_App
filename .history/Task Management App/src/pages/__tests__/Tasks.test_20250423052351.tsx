...
describe('Tasks Component', () => {
  ...
  // ทดสอบเมื่อการสร้าง task ล้มเหลว
test('shows error when create task fails', async () => {
    render(<Tasks />);
  
    // รอให้ข้อความแสดงขึ้นมาใน DOM
    await waitFor(() => {
      const errorMessage = screen.getByText((content) =>
        content.includes('failed to create task')
      );
  ...
  test('redirects to login if no email found in localStorage', () => {
  const navigate = jest.fn();
  useNavigate.mockReturnValue(navigate);

  // เราจะทำการ render component ภายใน MemoryRouter เพื่อให้สามารถใช้งานการนำทางได้
  render(
    <MemoryRouter>
      <Tasks />
    </MemoryRouter>
  ...
  // ตรวจสอบว่า navigate ถูกเรียกด้วยเส้นทาง '/' (หน้า login)
