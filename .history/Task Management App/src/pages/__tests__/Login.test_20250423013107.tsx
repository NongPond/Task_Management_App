...
import Register from '../../auth/Register';  // Ensure the path is correct
...
describe('Login component', () => {
  ...
  it('should display validation errors for invalid input', async () => {
    render(<Login />); // Now using the Login component for validation testing

    fireEvent.click(screen.getByRole('button', { name: /login/i })); // Adjust for the login button
  ...
  it('should show an alert when the login request fails', async () => {
    ...
    await waitFor(() => {
      const alertText = screen.getByText((content, element) =>
        content.includes('อีเมลหรือรหัสผ่านไม่ถูกต้อง') && element.tagName === 'P'
      );
