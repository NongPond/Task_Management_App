...
describe('Tasks Component', () => {
  ...
  it('does not allow task update without title or description', async () => {
    ...
    const errorMessage = await screen.findByRole('alert');
    expect(errorMessage).toHaveTextContent(/at least a title or description is required/i);
  });

  it('prevents task deletion if task not found', async () => {
    server.use(
      rest.delete('http://localhost:5000/api/tasks/:id', (_, res, ctx) => {
        return res(ctx.status(404)); // Simulate task not found error
      })
    );
    ...
    const errorMessage = await screen.findByRole('alert');
    expect(errorMessage).toHaveTextContent(/error deleting task/i);
  });
