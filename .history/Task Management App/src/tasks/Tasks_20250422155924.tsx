...
const taskSchema = Yup.object({
  title: Yup.string()
    .required('กรุณาใส่ชื่อของงาน')
    .min(3, 'ชื่องานต้องมีอย่างน้อย 3 ตัวอักษร'),
  description: Yup.string()
    .required('กรุณาใส่รายละเอียดของงาน')
    .min(5, 'รายละเอียดต้องมีอย่างน้อย 5 ตัวอักษร'),
  status: Yup.string()
    .oneOf(['pending', 'in-progress', 'completed'], 'สถานะไม่ถูกต้อง')
    .required('กรุณาเลือกสถานะของงาน'),
});
...
export default function Tasks() {
  ...
  const handleCreate = async (values: Omit<Task, '_id'>, resetForm: () => void) => {
    try {
      ...
      <div style={{ background: '#f9f9f9', padding: '1rem', borderRadius: '10px', marginBottom: '2rem' }}>
        ...
        >
          <Form style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
            <div>
              <Field name="title" placeholder="Title" style={inputStyle} />
              <ErrorMessage name="title" component="div" style={{ color: 'red' }} />
            </div>
            <div>
              <Field name="description" placeholder="Description" style={inputStyle} />
              <ErrorMessage name="description" component="div" style={{ color: 'red' }} />
            </div>
            <div>
              <Field name="status" as="select" style={inputStyle}>
