...
export default function Tasks() {
  ...
  const handleUpdate = async (_id: string, updates: Partial<Task>) => {
    // ตรวจสอบเฉพาะกรณีที่มีการแก้ไข title หรือ description เท่านั้น
    if ((updates.title && !updates.title.trim())) {
      setFormError('Title is required');
      return;
    }
  
    setFormError(null); // ล้าง error ถ้าผ่าน validation
  
    try {
      const res = await fetch(`http://localhost:5000/api/tasks/${_id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ status }), // ส่งแค่ status ที่ต้องการ
      });
