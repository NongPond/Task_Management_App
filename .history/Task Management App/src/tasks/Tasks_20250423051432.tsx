...
export default function Tasks() {
  ...
  const handleCreate = async () => {
    if (!newTask.title.trim() || !newTask.description.trim()) {
      setFormError('ทั้งชื่อและคำอธิบายจำเป็นต้องกรอก');
      return;
    }
    ...
    try {
      ...
      setError('ไม่สามารถสร้างงานได้ กรุณาลองใหม่อีกครั้ง');
  ...
  const handleUpdate = async (_id: string, updates: Partial<Task>) => {
    if (!updates.title?.trim() && !updates.description?.trim() && !updates.status) {
      setFormError('ต้องมีชื่อ, คำอธิบาย หรือสถานะ');
      return;
    }
  
    // ตรวจสอบการอัพเดตสถานะ
    if (updates.status && !['pending', 'in-progress', 'completed'].includes(updates.status)) {
      setFormError('สถานะไม่ถูกต้อง');
      return;
    }
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem(`tasks-${email}`);  // ลบข้อมูล task ออกจาก localStorage
    navigate('/');
  };
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '800px', margin: '0 auto', fontFamily: 'Segoe UI', background: '#f9f9f9' }}>
      ...
      {formError && <div role="alert" style={{ background: '#f8d7da', color: '#721c24', padding: '1rem', borderRadius: '8px', marginBottom: '1rem' }}>
        {formError}
      </div>}
