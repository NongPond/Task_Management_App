...
export default function Tasks() {
  ...
  const handleCreate = async () => {
    ...
    if (!newTask.description.trim()) {
      setError('Description is required'); // Set the error message
      return;
    }
    ...
    try {
      ...
      setError(null); // Clear the error after successful task creation
    } catch (err) {
      console.error('Error creating task:', err);
      setError('Failed to create task'); // Set error if creation fails
    }
  };
  ...
  const handleUpdate = async (_id: string, updates: Partial<Task>) => {
    try {
      ...
      setError('Failed to update task'); // Set error if update fails
    }
  };
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '800px', margin: '0 auto', fontFamily: 'Segoe UI', background: '#f9f9f9' }}>
      ...
      {error && <div role="alert" style={{ color: 'red', marginBottom: '1rem' }}>{error}</div>} {/* Display error message */}
