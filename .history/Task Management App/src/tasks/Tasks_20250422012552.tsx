...
export default function Tasks() {
  ...
  const handleCreate = () => {
    ...
    setNewTask({ title: '', description: '', status: 'pending', _id: '' });
  };

  const handleUpdate = (id: string, updates: Partial<Task>) => {
    const updated = tasks.map(task => task._id === id ? { ...task, ...updates } : task);
    setTasks(updated);
    setEditingId(null);
  };
          ...
          {editingId === task._id ? (
            <>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <button style={buttonPrimary} onClick={() => handleUpdate(task., editedTask)}>Save</button>
