...
describe('Tasks Component', () => {
  ...
  it('renders multiple tasks when API returns many', async () => {
    const twoTasks = [...mockTasks, { _id: '2', title: 'Second Task', description: 'Desc 2', status: 'completed' }];
  ...
  it('resets edited fields when cancel is clicked', async () => {
    render(<MemoryRouter><Tasks /></MemoryRouter>);
    fireEvent.click(await screen.findByRole('button', { name: /edit/i }));
    const input = screen.getByDisplayValue(/test task/i);
    await userEvent.clear(input);
    await userEvent.type(input, 'Temp Edit');
    fireEvent.click(screen.getByRole('button', { name: /cancel/i }));
    expect(screen.getByText('Test Task')).toBeInTheDocument();
  });
  ...
  it('loads tasks from localStorage if present', async () => {
    localStorage.setItem(`tasks-${email}`, JSON.stringify(mockTasks));
    render(<MemoryRouter><Tasks /></MemoryRouter>);
    expect(await screen.findByText('Test Task')).toBeInTheDocument();
  });
